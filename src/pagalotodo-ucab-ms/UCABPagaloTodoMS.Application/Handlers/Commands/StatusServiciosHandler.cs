...
namespace UCABPagaloTodoMS.Application.Handlers.Commands
{
    ...
    */
    public class StatusServiciosHandler : IRequestHandler<StatusServiciosCommand, string>
     {
         private readonly IUCABPagaloTodoDbContext _dbContext;
         private readonly ILogger<StatusServiciosCommand> _logger;

         public StatusServiciosHandler(IUCABPagaloTodoDbContext dbContext,ILogger<StatusServiciosCommand> logger)
         {
        ...
        */
        ...
        public Task<string> Handle(StatusServiciosCommand request, CancellationToken cancellationToken)
         {
             try
             {
        ...
        <returns>Returns the username of the updated status</returns>    
        */
        private async Task<string> HandleAsync(StatusServiciosCommand request)
         {
            try
            {
               //A instance to check the info's format with the Validator
               var validator = new StatusServicioValidator();
               //Check the info's format and is stored on result
               ValidationResult result = validator.Validate(request);
                if (result.IsValid)
                {
                    return await ServicioValidation.ValidateStatusServicio(request, _dbContext);
                }
                else
                {
